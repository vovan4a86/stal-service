...
class AdminSettingsController extends AdminController {
	...
	public function postSave()
	{
		...
		if (empty($_FILES)) {
			...
			return ['redirect' => redirect(route('admin.settings.groupItems', [$group_id]))];
		}
	}
			...
			case 3:
				if (starts_with($value, 'setting_file_')) {
					...
					if ($file = Request::file($value)) {
                        //другое имя файла, если это прайс-лист
                        if($setting->id == 33) {
                            $file_name = 'stal-service-price' . '.' . $file->getClientOriginalExtension();
                        } else {
                            $file_name = 'setting_' . $setting->id . '.' . $file->getClientOriginalExtension();
                        }
