<form action="{{ $category->url }}" id="setView">
    <div class="togglers">
        <span class="togglers__toggle {{ session('catalog_view', 'grid') == 'grid' ? 'togglers__toggle--active': '' }}"
        ...
        <span class="togglers__toggle {{ session('catalog_view', 'grid') == 'list' ? 'togglers__toggle--active': '' }} "
